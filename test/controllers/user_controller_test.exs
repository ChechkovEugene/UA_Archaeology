defmodule UaArchaeology.UserControllerTest do
  ...
  test "redirects from new form when not admin", %{conn: conn, nonadmin_user: nonadmin_user} do
    ...
    conn = get conn, user_path(conn, :new)
    assert get_flash(conn, :error) == "Ви не авторизовані для створення нових користувачів!"
  ...
  test "redirects away from editing when logged in as a different user", %{conn: conn, nonadmin_user: nonadmin_user, admin_user: admin_user} do
    ...
    assert get_flash(conn, :error) == "Ви не авторизовані для зміни цього користувача!"
    assert redirected_to(conn) == page_path(conn, :index)
    assert conn.halted
  end
